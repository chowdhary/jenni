...
def add_definition(jenni, word, definition, username, created):
    #global con
    try:
        con = sql.connect('clanfuq.db')
        cur = con.cursor()
...
def clanfuq(jenni, input):
    ...
    if len(words) == 1 and words[0].endswith("?"):
        word = words[0].rstrip("?")
        get_definition(jenni, word)
    elif len(words) > 2 and words[1] == "is":
        username = input.nick
        created = int(time.time())
        add_definition(jenni, words[0], text, username, created)
clanfuq.rule = r'.*'
